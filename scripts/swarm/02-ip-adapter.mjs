// Swarm agent: IP-Adapter Face ID — SDXL face-preserving generation
// v1 — 2026-02-20
...
async function generate(style, faceUrl) {
  try {
    ...
    return result.data?.image?.url || result.data?.images?.[0]?.url || null;
  } catch (err) {
    console.error(`  [${MODEL_NAME}/${style.name}] Error: ${err.message}`);
    if (err.body) console.error(`    Detail: ${JSON.stringify(err.body).slice(0, 200)}`);
    return null;
  }
}
