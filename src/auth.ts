...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        // Check access_requests for this email with a stored password hash
        const snap = await db
  ...
  callbacks: {
    async signIn({ user }) {
      ...
      if (snap.empty) {
        // Redirect to pending page instead of generic error
    ...
    async jwt({ token, user }) {
      if (user?.email) {
        ...
        token.approved = true; // If they got past signIn, they're approved
      }
      return token;
    },
